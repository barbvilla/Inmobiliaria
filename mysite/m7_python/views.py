...
def register_tipoView(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            tipo_user = TipoUser.objects.filter(id=int(tipo))[0]
...
def new_inmuebleView(request):
    if request.method == 'POST':
        ...
        if u_form.is_valid():
            ...
            id_tipo_inmueble = TipoInmueble.objects.filter(id=int(id_tipo_inmueble))[0]
            ...
            inm = Inmuebles(id_tipo_inmueble=id_tipo_inmueble,
                            id_comuna = comuna,
                            id_region = reg,
